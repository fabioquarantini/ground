	...
	threeAnimation(item) {
		...
		const scrollAnimate = (model) => {
			...
			tl
				...
				// .to(model.rotation, { y: 0.2 })
				// .to(model.rotation, { x: 0.5, z: 2, y:-4 })

				// .from(camera.position, { y: 2, z: 2, x: 5 })
				// .to(camera.position, { y: -2, z: 1, x: -5 }, 'step2')
				.from(model.rotation, { z: 1 }, 'step1')
				.to(scene.rotation, { y: 0.5 }, 'step1')
				.to(model.position, { z: 4 }, 'step2');
		};
	}
