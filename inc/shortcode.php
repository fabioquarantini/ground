 ...
 * Google maps
 *
 * [map aspect-w="16" aspect-h="9" width="200" height="200" src="[url]"] or [map src="[url]"]
 *
 * @param array  $atts An array of attributes.
 * @param string $content The shortcode content or null if not set.
 * @return string Shortcode output.
...
function ground_shortcode_map( $atts, $content = null ) {
	...
	if ( ! isset( $atts['ratio'] ) ) {
		return '<iframe frameborder="0" scrolling="no"width="' . esc_html( $atts['width'] ) . '" height="' . esc_html( $atts['height'] ) . '"  marginheight="0" marginwidth="0" src="' . esc_html( $atts['src'] ) . '&amp;output=embed"></iframe>';
	} else {
		return '<div class="aspect-w-' . esc_html( $atts['aspect-w'] ) . ' aspect-h-' . esc_html( $atts['aspect-h'] ) . '"><iframe frameborder="0" scrolling="no" marginheight="0" marginwidth="0" src="' . esc_html( $atts['src'] ) . '&amp;output=embed"></iframe></div>';
	}
